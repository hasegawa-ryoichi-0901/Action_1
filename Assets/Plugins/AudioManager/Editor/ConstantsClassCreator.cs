...
namespace KanKikuchi.AudioManager {
    ...
    public static class ConstantsClassCreator {
        ...
        /// 定数を管理するクラスを自動生成する
        /// </summary>
        public static void Create<T>(string className, string summary, Dictionary<string, T> valueDict,
                                     string exportDirectoryPath, string nameSpace = "") {
            //入力された型の判定
            ...
            builder.AppendFormat($"public enum {className}Type").AppendLine("{").AppendLine();
            builder.AppendLine("NONE = -1,").AppendLine();
            //入力された定数とその値のペアを書き出していく
            string[] keyArray = newValueDict.Keys.ToArray();
            foreach (string key in keyArray) {
                builder.AppendFormat($"{key},").AppendLine();
            }
            ...
            foreach (string key in keyArray) {
                ...
                else if (!Regex.IsMatch(key, @"^[_a-zA-Z0-9]+$")) {
                    continue;
                }
            ...
            builder.AppendLine($"public static Dictionary<{className}Type, string> Dic = new() {{");
            foreach (var kv in newValueDict) {
                builder.AppendLine($"{{{className}Type.{kv.Key},\"{kv.Value}\"}},");
            }
            builder.AppendLine("};");
            builder.AppendLine().AppendLine("}");
