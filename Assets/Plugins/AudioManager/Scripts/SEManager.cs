...
public class SEManager : AudioManager<SEManager> {
  ...
  /// 再生
  /// </summary>
  public AudioPlayer Play(SEPath.SEPathType seType, float volumeRate = 1, float delay = 0, float pitch = 1, bool isLoop = false, Action callback = null) {
    if (!SEPath.Dic.TryGetValue(seType, out var v)) {
      return null;
    }
    volumeRate = AdjustVolumeRate(volumeRate, v);
    if (volumeRate > 0) {
